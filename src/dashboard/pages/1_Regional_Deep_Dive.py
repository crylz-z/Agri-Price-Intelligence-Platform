...
if selected_date_str not in available_dates:
    ...
    if closest_dates:
        ...
        if is_today:
            st.warning(
                f"🕒 **Pending Extraction**: Market data for `{selected_date_str}` is not yet available (likely pending daily update or server outage). Falling back to the latest data from `{actual_date_str}`."
            )
        else:
            st.warning(
