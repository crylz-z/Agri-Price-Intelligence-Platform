...
def render_kpi_cards(commodity_df, trend_df=None):
    ...
    with c1:
        with st.container(border=True):
            st.metric("Avg Price", f"₱{avg_price:,.2f}", delta=price_delta, delta_color="inverse", help=f"Average prevailing price for {full_comm_name}")
            
    with c2:
        with st.container(border=True):
            st.metric("Best Deal", f"₱{cheapest_price:,.2f}", f"{cheapest_market}", help="Lowest price currently available")
            
    with c3:
        with st.container(border=True):
            st.metric("Markets Reporting", market_count, help=f"Number of markets reporting price for {full_comm_name}")
...
def render_sparklines(trend_df, category_name):
    ...
    chart = alt.Chart(trend_df).mark_area(
        ...
        color=alt.Gradient(
            gradient='linear',
            stops=[alt.GradientStop(color='#2E86AB', offset=0),
        ...
        y=alt.Y('Prevailing Price (₱):Q', title=None, scale=alt.Scale(domain=[min_val, max_val]), axis=None),
...
def render_zscore_chart(df, height=400):
    ...
    chart = alt.Chart(df).mark_bar().encode(
        ...
        color=alt.Color('z_score:Q', scale=alt.Scale(scheme='redyellowgreen', domain=[2, -2]), legend=None),
