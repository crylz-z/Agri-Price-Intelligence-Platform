...
def render_kpi_cards(commodity_df):
    """
    Renders the top row of KPI cards using COMPONENT-LEVEL data.
    1. Avg Price (Specific Commodity)
    ...
    # Best Deal Logic (Strictly filtered by commodity_df now)
    ...
    # Unit extraction (naive attempt to display context)
    # The commodity name itself often has the unit: "Red Onion (kg)"
    # We will display the full commodity name in the Best Deal card to show unit.
    full_comm_name = cheapest_row['commodity']
    
    # Render
    c1, c2, c3 = st.columns(3)
    c1.metric("Avg Price", f"₱{avg_price:,.2f}", f"{full_comm_name}")
    c2.metric("Best Deal", f"₱{cheapest_price:,.2f}", f"{cheapest_market}")
    c3.metric("Markets Reporting", market_count, help=f"Number of markets reporting price for {full_comm_name}")
