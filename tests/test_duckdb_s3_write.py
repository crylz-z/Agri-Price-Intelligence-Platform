...
try:
    ...
    df = con.sql(
        f"SELECT * FROM read_csv_auto('s3://{bucket}/bronze/year=2026/month=02/day=17/*.csv', union_by_name=true) LIMIT 5"
    ).df()
...
try:
    con.execute(
        f"COPY (SELECT * FROM read_csv_auto('s3://{bucket}/bronze/year=2026/month=02/day=17/*.csv', union_by_name=true) LIMIT 10) TO '{test_s3_path}' (FORMAT PARQUET)"
    )
...
try:
    verify_df = con.sql(
        f"SELECT COUNT(*) as cnt FROM read_parquet('{test_s3_path}')"
    ).df()
